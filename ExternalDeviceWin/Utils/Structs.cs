...
namespace ExternalDeviceWin.Utils
{
    ...
    {
        public short version;       //USBIP version
        public ushort command;      //Command code: Retrieve the list of exported USB devices.
        public int status;          //Status: unused, shall be set to 0
    ...
    [StructLayout(LayoutKind.Sequential, Pack = 1)]
    public unsafe struct OP_REP_DEVLIST
    {
        public short version;
        public short repCode;
        public int status;      //Status: 0 for OK
        public int exportedDevices;//Number of exported devices: 0 means no exported devices.
        public fixed byte path[256];//From now on the exported n devices are described, if any. If no devices are exported the message ends with the previous “number of exported devices” field.
        public fixed byte busid[32];//busid: Bus ID of the exported device, string closed with zero byte, e.g. “3-2”. The unused bytes shall be filled with zero bytes.
        public int busnum;
        public int devnum;
        public int speed;
        public short idVender;
        ...
        public byte alignment;//padding byte for alignment, shall be set to zero
