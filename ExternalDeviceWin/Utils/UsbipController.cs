...
{
    public class UsbipController : IDisposable
    {
        ...
        /// <summary>
        /// just for hid devices, no need in printer
        /// </summary>
        ...
        public int Port = 3240;

        public bool Connected
        {
            get { return _clntSocket is not null; }
        }

        public bool VerboseOutput { get; set; }

        public UsbipController(USB_DEVICE_DESCRIPTOR desc, CONFIG_HID configuration_hid, byte[] report_descriptor)
        {
            _desc = desc;
        ...
        public UsbipController(USB_DEVICE_DESCRIPTOR desc)
        {
            _desc = desc;
            _report_descriptor = Array.Empty<byte>();
        }
        ...
        private static Socket ConnectSocket(IEnumerable<byte> serverIPAddress, int port)
        {
            Socket? s = null;
            IPHostEntry? hostEntry = null;
            
            hostEntry = Dns.GetHostEntry(new IPAddress(serverIPAddress.Select(i => i).ToArray()));
            ...
            return s ?? throw new NullReferenceException();
        }
        ...
        void handle_data(Socket clntSocket, USBIP_RET_SUBMIT usb_req, int bl)
        {
            while (true)
            {
                if (_queue.TryDequeue(out var result) == false)
                {
                    Thread.Sleep(16);
                    continue;
                }
        ...
        unsafe int handle_get_descriptor(Socket clntSocket, USB_DEVICE_DESCRIPTOR desc, CONFIG_HID? configuration_hid, StandardDeviceRequest control_req, USBIP_RET_SUBMIT usb_req)
        {
            ...
            if (control_req.wValue1 == 0x2) // configuration
            {
                ...
                if (configuration_hid is not null)
                {
                    buf = StructureToBytes<CONFIG_HID>(configuration_hid ?? throw new NullReferenceException());
                }
        ...
        unsafe void send_usb_req(Socket clntSocket, USBIP_RET_SUBMIT usb_req, byte[]? data, uint size, uint status)
        {
            ...
            fixed (byte* pByte = buf)
            {
                ...
                HelperExtension.pack(intBytes, sizeof(USBIP_RET_SUBMIT));
            }
